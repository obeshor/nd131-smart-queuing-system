...
class PersonDetect:
    ...
    def predict(self, frame, initial_w, initial_h):
        ...
        async_infer_handler = self.net.start_async(request_id=0, inputs=input_dict)
        
        # Wait for the result
        if async_infer_handler.wait() == 0:
            # Get the results of the inference request
            net_output = async_infer_handler.outputs[self.output_name]
            
        image, coords = self.draw_outputs(net_output, frame, initial_w, initial_h)
